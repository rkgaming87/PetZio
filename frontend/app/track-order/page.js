...
function TrackOrderContent() {
  ...
  const fetchTracking = async (idToFetch) => {
    const targetId = idToFetch || orderId;
    if (!targetId.trim()) {
    ...
    setIsTracking(true);
    try {
      const response = await api.get(`/orders/${targetId.trim()}`);
        ...
        <form onSubmit={handleTrack} className="flex gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="e.g. 6a9fc40bd1e53f9a78d41f14"
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <div className="py-12 px-4 sm:px-6 lg:px-8">
        <Suspense fallback={<div className="text-center">Loading tracker...</div>}>
          <TrackOrderContent />
        </Suspense>
      </div>
    </div>
