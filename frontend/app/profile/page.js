                  ...
                  <div className="space-y-4">
                    {orders.map(order => (
                      <div key={order._id} className="p-4 bg-neutral-50 dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
                        <div>
                          <p className="font-semibold text-neutral-900 dark:text-white">Order #{order._id.substring(order._id.length - 8).toUpperCase()}</p>
                          <p className="text-sm text-neutral-500 mt-1">Date: {new Date(order.order_date).toLocaleDateString()}</p>
                          <p className="text-xs text-neutral-400 mt-0.5">Items: {order.items?.length || 0}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-indigo-600 dark:text-indigo-400">₹{order.total_amou.toFixed(2)}</p>
                          <button onClick={() => router.push(`/track-order?orderId=${order._id}`)} className="text-sm text-indigo-600 dark:text-indigo-400 font-medium hover:underline mt-1 block">Track Order</button>
                        </div>
                      </div>
                    ))}
                  </div>
